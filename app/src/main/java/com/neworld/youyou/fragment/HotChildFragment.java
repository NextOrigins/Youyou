...
public class HotChildFragment extends BaseFragment {
    ...
    public View createView() {
        View view = View.inflate(context, R.layout.hot_child, null);
...
//        hotEveryAdapter = initAdapter(hotEveryAdapter);
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (createDate != null && createDate.length() > 0) {
                                String base64 = Base64.encodeToString(("{\"createDate\": \""+createDate+"\",\"type\": \"4\",\"userId\": \""+userId+"\"}").getBytes(), Base64.DEFAULT);
    ...
    public Object getData() {
        String base64 = Base64.encodeToString(("{\"createDate\": \"\",\"type\": \"4\",\"userId\": \""+userId+"\"}").getBytes(), Base64.DEFAULT);
        String replace = base64.replace("\n", "");
        String content = NetManager.getInstance().getContent(replace, "131");
        if (content != null && content.length() > 0) {
            HotBean hotBean = GsonUtil.parseJsonToBean(content, HotBean.class);

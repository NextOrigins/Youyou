...
public class HotYoungFragment extends BaseFragment {
...
//        hotEveryAdapter = initAdapter(hotEveryAdapter);
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (createDate != null && createDate.length() > 0) {
                                String base64 = Base64.encodeToString(("{\"createDate\": \""+createDate+"\",\"type\": \"2\",\"userId\": \""+userId+"\"}").getBytes(), Base64.DEFAULT);
    ...
    public Object getData() {
        String base64 = Base64.encodeToString(("{\"createDate\": \"\",\"type\": \"2\",\"userId\": \""+userId+"\"}").getBytes(), Base64.DEFAULT);
        String replace = base64.replace("\n", "");
        String content = NetManager.getInstance().getContent(replace, "131");
        if (content != null && content.length() > 0) {
            final HotBean hotBean = GsonUtil.parseJsonToBean(content, HotBean.class);
            if (hotBean != null && hotBean.getStatus() == 0) {
                menuList = hotBean.getMenuList();
                if (menuList != null) {
                    mData.clear();

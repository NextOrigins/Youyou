...
public class JuniorFragment extends BaseFragment implements View.OnClickListener, FragmentBackHandler {
    ...
    private List<AddClassBean.MenuListBean> classMenuList;

    @Override
    public View createView() {
        View view = View.inflate(context, R.layout.fragment_school_junior, null);
    ...
    /*    if (tvChildSchool.getText().toString().trim() != null && tvChildSchool.length() > 0) {
        ...
        /*if (tvChildLocation.getText().toString().trim() != null && tvChildLocation.length() > 0) {
            ...
            if (activity.locationJunior != null && activity.locationJunior.length() > 0) {
                tvChildLocation.setText(activity.locationJunior);
            } else {
      ...
      /*  if (tvChildTime.getText().toString().trim() != null && tvChildTime.length() > 0){
            ...
            if (activity.timeJunior != null && activity.timeJunior.length() > 0 ) {
                tvChildTime.setText(activity.timeJunior);
            } else {
                //tvChildTime.setText("");
            }
        //}
            ...
            if (activity.gradeJunior != null && activity.gradeJunior.length() > 0) {
                tvChildLevel.setText(activity.gradeJunior);
            } else {
    ...
    private void initLevelNet() {
        if (tvChildSchool.getText().toString().trim() != null && tvChildSchool.length() > 0) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    String base64 = Base64.encodeToString(("{\"type\":\"2\", \"schoolName\":\""+tvChildSchool.getText().toString().trim()+"\"}").getBytes(), Base64.DEFAULT);
    ...
    private void initSchoolNet() {
        if (tvChildLocation.getText().toString().trim() != null && tvChildLocation.length() > 0) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    String base64 = Base64.encodeToString(("{\"type\":\"2\", \"districtName\":\""+tvChildLocation.getText().toString().trim()+"\"}").getBytes(), Base64.DEFAULT);
                    String replace = base64.replace("\n", "");
        ...
        pvOptionsQu = new OptionsPickerView.Builder(context, new OptionsPickerView.OnOptionsSelectListener() {
            ...
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                ...
                if (location != null) {
                    ...
                    //activity.locationJunior = location;
                    if (activity.locationJunior != location) {
                        tvChildSchool.setText("");
                        //tvChildTime.setText("");
    ...
    /*    if (activity.locationJunior.length() > 0 && activity.junior.length() > 0 && activity.timeJunior.length() > 0 && activity.gradeJunior.length() > 0) {
            tanDialog(false);
        } else if (activity.locationJunior.length() == 0 && activity.junior.length() == 0 && activity.timeJunior.length() == 0 && activity.gradeJunior.length() == 0){
            activity.popBackStack();
        } else {
            tanDialog(true);
        }*/
            ...
            public void run() {
                ...
                if (babyId != 0) {
                    String base64 = Base64.encodeToString(("{\"babySchoolId\":\"\",\"babyId\":\""+babyId
                            ...
                            +"\",\"grade_type\":\""+level+"\", \"type\":\"2\", \"schoolName\":\""+school+"\"}").getBytes(), Base64.DEFAULT);
                    String replace = base64.replace("\n", "");
                    ...
                    if (content != null && content.length() > 0) {
                        ...
                        if (returnStatus != null && returnStatus.getStatus() == 0) {
                            Util.uiThread(new Runnable() {
                                @Override
                                public void run() {
                                    activity.junior = school;
                                    activity.school = school;
                                    activity.locationJunior= location;
                                    activity.timeJunior= time;
                                    activity.gradeJunior= level;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (location != null && location.length() > 0) {
                    String base64 = Base64.encodeToString(("{\"type\":\"2\", \"districtName\":\""+location+"\"}").getBytes(), Base64.DEFAULT);
                    String replace = base64.replace("\n", "");
                    String content = NetManager.getInstance().getContent(replace, "166");
                    if (content != null && content.length() > 0) {
                        SchoolNameBean schoolNameBean = GsonUtil.parseJsonToBean(content, SchoolNameBean.class);
                   ...
                   pvOptionsSchool = new OptionsPickerView.Builder(context, new OptionsPickerView.OnOptionsSelectListener() {
                       ...
                       public void onOptionsSelect(int options1, int options2, int options3, View v) {
                           ...
                           if (pickerViewText != null && pickerViewText.length() > 0) {
                               ...
                               //activity.junior = pickerViewText;
    ...
    private void getNetGrade(final String pickerViewText) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String base64 = Base64.encodeToString(("{\"type\":\"2\", \"schoolName\":\""+pickerViewText+"\"}").getBytes(), Base64.DEFAULT);
                String replace = base64.replace("\n", "");
                String content = NetManager.getInstance().getContent(replace, "167");
                if (content != null && content.length() > 0) {
                    AddClassBean addClassBean = GsonUtil.parseJsonToBean(content, AddClassBean.class);
